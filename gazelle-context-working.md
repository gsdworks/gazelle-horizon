...
**Decision: set an order ID prefix in Shopify (Settings → Checkout → order ID format) so orders mint as `#GZ1001`, then send `order.name` verbatim in field A.** That gets collision-proofing AND a native Matrixify match with no transform, because the value Fred stores and echoes back IS the Shopify order name.
...
- **Set the `GZ` order ID prefix** (Settings → Checkout). **Before any test order exists.**
- **Matrixify stock import profile** — fixed path `/Stock/GazelleAvailability.csv`, match on **Variant Barcode**, inventory qty only, **update-present-only / never zero an absent EAN**, schedule **02:00**. Fully specced, clean sample in hand, nothing outstanding.
...
- [ ] **Set the `GZ` order ID prefix** (Settings → Checkout) — **before any test order exists** (PO collision: Fred's POs run 3595–4252, Shopify starts at 1001)
...
- [ ] Daily availability file — ✅ **arrived + confirmed clean (9 Jul); coverage confirmed (16 Jul) = all 978/979 EANs at status 00/01**, `0` rows proven in data, no negatives, 01:29 daily. **⚠️ Point Matrixify at the FIXED name `/Stock/GazelleAvailability.csv`, NOT the dated file** (Fred added fixed-name copies 16 Jul; a rotating filename breaks a scheduled import). REMAINING: configure the **update-present-only / never-zero-absent** rule
...
- ~~Stock file (in flight)~~ **✅ DONE — arrived, clean, coverage confirmed (all 00/01), fixed-name copy in place.** Our side: set Matrixify to **update-present-only, never zero absent EANs**, pointed at the **fixed** filename.
