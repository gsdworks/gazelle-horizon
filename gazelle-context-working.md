...
- **FRED** — stock export send changed-only (delta) not the full 54,517 — targeted early this week; **explicitly NOT a go-live blocker** (full-file path already proven) — awaiting since 20 Jul (7 days)
- **BILLY** — owed a **BooksoniX outstanding-bits list (today)** + a product-page preview; go-live reframed off a date to "chase Dave with me" — send today (NEW 27 Jul)
- **OURS** — barcode-uniqueness audit in the store before go-live — source now confirmed **single across 54,517 rows, no dupes** (Lee removed the fudged `9781909360266`); store-side audit still to run
...
- **OURS** — verify one-email-per-parcel with notifications ON before enabling send-receipt — Billy wants send-receipt **TRUE**; hold FALSE until launch, then this check gates the flip — not started
- **DAVE** — force-update propagation (the real go-live gate) + archive-not-delete, Thema two-format, two-halves catalogue, approved-only filter, PUBCODE metafield — **STILL OPEN:** fresh 1,734-product export 27 Jul shows **897 (~52%) still `%sh_height_mm%`**, **860/1,172 populated Thema still decoded-names not codes**, **product type blank on 1,062**, **only 14/1,734 COMPLETE** (crept ~200 clean since 16 Jul); holding 48h before chasing BooksoniX; Paul Theijs back from OOO today (27 Jul)
...
- **Verify against source BEFORE flagging — and confirm a column is POPULATED before reading its absence as a finding** (repeat offender: A–N wrong-file, combine-list unread email, barcode apostrophe, two more on 20 Jul, + the 27 Jul Tags column that didn't export at all — 0/1,734 rows read as "0% tagged" when coverage was unreadable, not zero; next product export MUST include Tags).
