...
2. **Reconcile OPEN LOOPS (Section 3).** For each item: **advance** / **close** / or **flag STALE** with days waiting.
...
- **FRED** — strip the header space — awaiting reply — since 20 Jul
- **FRED** — duplicate barcode `9781909360266`, is it 80 or 0 — awaiting reply — since 20 Jul
- **FRED** — can the stock export send changed-only (delta) not the full 54k — awaiting reply — since 20 Jul
- **FRED** — does a single title's QUANTITY ever ship split across parcels (3 of 7)? if yes we add + TEST a `Line: Quantity` column — awaiting reply — since 20 Jul
- **BILLY** — send-receipt TRUE/FALSE (per-parcel customer email) — awaiting reply — since 20 Jul
